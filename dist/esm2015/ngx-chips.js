...
import { filter, first, debounceTime, switchMap, map } from 'rxjs/operators';
...
import { VirtualScrollComponent, VirtualScrollModule } from 'angular2-virtual-scroll';
...
class TagInputDropdown {
     ...
     */
    get selectedItem() {
        return /** @type {?} */ ((this.dropdown.menu.state.dropdownState.selectedItem));
    }
    /**
     * \@name state
     * @return {?}
...
class TagInputVirtualizedDropdown {
    /**
     * @param {?} injector
     */
    constructor(injector) {
        ...
        this.show = () => {
            ...
            const /** @type {?} */ showDropdownIfEmpty = this.showDropdownIfEmpty && hasItems && !value;
            const /** @type {?} */ isDisabled = this.tagInput.disable;
            const /** @type {?} */ shouldShow = isHidden && ((hasItems && hasMinimumText) || showDropdownIfEmpty || this.showDropdownAlways);
            const /** @type {?} */ shouldHide = this.isVisible && !hasItems;
            if (this.flexibleMenuHeight) {
                const /** @type {?} */ el = this.dropdown.menu['element']['nativeElement']['children'][0];
                const /** @type {?} */ totalHeight = items.length * this.dropdownMenuItemHeight;
                if (el.style.height) {
                    el.style.cssText.replace(/(height\:)\s*[0-9]+(px|%)\s*(!important)?;?/i, '');
                }
                const /** @type {?} */ newHeight = totalHeight < 400 ? (totalHeight + 10).toString() + 'px' : '100%';
         ...
         * @param text
         */
        this.getItemsFromObservable = (text, skip, limit) => {
            this.setLoadingState(true);
            const /** @type {?} */ subscribeFn = (data) => {
                // hide loading animation
                this.setLoadingState(false)
    ...
    ngOnInit() {
        ...
        this.tagInput
            .onTextChange
            .asObservable()
            .pipe(debounceTime(200), filter((text) => !this.keepOpen
            ? text.trim().length > 0
            : true))
            .subscribe(this.show);
        if (this.totalOfItemsObservable) {
            this.vScroll
                .end
                .asObservable()
                .pipe(debounceTime(350), filter((e) => {
                if (e.end === undefined) {
                    return false;
                }
                const /** @type {?} */ autocompleteItemsCount = this.autocompleteItems.length - this.tagInput.items.length;
                const /** @type {?} */ scrolled = Math.floor((e.end * 100) / autocompleteItemsCount);
                return this.autocompleteItems.length > 0
                    && scrolled >= this.loadThresholdOfAutocompleteItems
                    && !this.tagInput.isLoading;
            }), switchMap(() => this.totalOfItemsObservable(this.tagInput.inputTextValue)), filter(total => this.autocompleteItems.length < total))
                .subscribe(() => this.getItemsFromObservable(this.tagInput.inputTextValue, this.autocompleteItems.length, this.autocompleteObservableFetchLimit));
        }
        this.dropdown
            .onShow
            .asObservable()
     ...
     */
    populateItems(data) {
        const /** @type {?} */ formattedItems = data.map(item => {
            return typeof item === 'string' ? {
                [this.displayBy]: item,
                [this.identifyBy]: item
            } : item;
...
/** @nocollapse */
TagInputVirtualizedDropdown.ctorParameters = () => [
    { type: Injector, },
];
TagInputVirtualizedDropdown.propDecorators = {
    "dropdown": [{ type: ViewChild, args: [Ng2Dropdown,] },],
    "vScroll": [{ type: ViewChild, args: [VirtualScrollComponent,] },],
    "templates": [{ type: ContentChildren, args: [TemplateRef,] },],
    "dropdownMenuItemWidth": [{ type: Input },],
    "dropdownMenuItemHeight": [{ type: Input },],
    "scrollbarWidth": [{ type: Input },],
    "scrollbarHeight": [{ type: Input },],
    "offset": [{ type: Input },],
    "focusFirstElement": [{ type: Input },],
    "showDropdownIfEmpty": [{ type: Input },],
    "showDropdownAlways": [{ type: Input },],
    "autocompleteObservable": [{ type: Input },],
    "loadThresholdOfAutocompleteItems": [{ type: Input },],
    "totalOfItemsObservable": [{ type: Input },],
    "autocompleteObservableFetchLimit": [{ type: Input },],
    "minimumTextLength": [{ type: Input },],
    "displayBy": [{ type: Input },],
    "identifyBy": [{ type: Input },],
    "matchingFn": [{ type: Input },],
...
const defaults$3 = forwardRef(() => OptionsProvider.defaults.tagInput);
class TagInputComponent extends TagInputAccessor {
    ...
    constructor(renderer, dragProvider) {
        ...
        this.applyFocusOnClick = true;
        this.applyFocusOnAdd = true;
        this.applyFocusOnRemove = true;
        this.applyFocusOnLast = true;
        /**
         * \@name separatorKeys
         * @desc keyboard keys with which a user can separate items
         */
        this.separatorKeys = new defaults$3().separatorKeys;
        /**
         * \@name separatorKeyCodes
         * @desc keyboard key codes with which a user can separate items
         */
        this.separatorKeyCodes = new defaults$3().separatorKeyCodes;
        /**
         * \@name placeholder
         * @desc the placeholder of the input text
         */
        this.placeholder = new defaults$3().placeholder;
        /**
         * \@name secondaryPlaceholder
         * @desc placeholder to appear when the input is empty
         */
        this.secondaryPlaceholder = new defaults$3().secondaryPlaceholder;
        /**
         * \@name maxItems
         * @desc maximum number of items that can be added
         */
        this.maxItems = new defaults$3().maxItems;
        /**
         * \@name validators
         * @desc array of Validators that are used to validate the tag before it gets appended to the list
         */
        this.validators = new defaults$3().validators;
        /**
         * \@name asyncValidators
         * @desc array of AsyncValidator that are used to validate the tag before it gets appended to the list
         */
        this.asyncValidators = new defaults$3().asyncValidators;
        /**
         * - if set to true, it will only possible to add items from the autocomplete
         * \@name onlyFromAutocomplete
         */
        this.onlyFromAutocomplete = new defaults$3().onlyFromAutocomplete;
        /**
         * \@name errorMessages
         */
        this.errorMessages = new defaults$3().errorMessages;
        /**
         * \@name theme
         */
        this.theme = new defaults$3().theme;
        /**
         * \@name onTextChangeDebounce
         */
        this.onTextChangeDebounce = new defaults$3().onTextChangeDebounce;
        /**
         * - custom id assigned to the input
         * \@name id
         */
        this.inputId = new defaults$3().inputId;
        /**
         * - custom class assigned to the input
         */
        this.inputClass = new defaults$3().inputClass;
        /**
         * - option to clear text input when the form is blurred
         * \@name clearOnBlur
         */
        this.clearOnBlur = new defaults$3().clearOnBlur;
        /**
         * - hideForm
         * \@name clearOnBlur
         */
        this.hideForm = new defaults$3().hideForm;
        /**
         * \@name addOnBlur
         */
        this.addOnBlur = new defaults$3().addOnBlur;
        /**
         * \@name addOnPaste
         */
        this.addOnPaste = new defaults$3().addOnPaste;
        /**
         * - pattern used with the native method split() to separate patterns in the string pasted
         * \@name pasteSplitPattern
         */
        this.pasteSplitPattern = new defaults$3().pasteSplitPattern;
        /**
         * \@name blinkIfDupe
         */
        this.blinkIfDupe = new defaults$3().blinkIfDupe;
        /**
         * \@name removable
         */
        this.removable = new defaults$3().removable;
        /**
         * \@name editable
         */
        this.editable = new defaults$3().editable;
        /**
         * \@name allowDupes
         */
        this.allowDupes = new defaults$3().allowDupes;
        /**
         * \@description if set to true, the newly added tags will be added as strings, and not objects
         * \@name modelAsStrings
         */
        this.modelAsStrings = new defaults$3().modelAsStrings;
        /**
         * \@name trimTags
         */
        this.trimTags = new defaults$3().trimTags;
        /**
         * \@name ripple
         */
        this.ripple = new defaults$3().ripple;
        /**
         * \@name tabindex
         * @desc pass through the specified tabindex to the input
         */
        this.tabindex = new defaults$3().tabIndex;
        /**
         * \@name disable
         */
        this.disable = new defaults$3().disable;
        /**
         * \@name dragZone
         */
        this.dragZone = new defaults$3().dragZone;
        /**
         * \@name onRemoving
         */
        this.onRemoving = new defaults$3().onRemoving;
        /**
         * \@name onAdding
         */
        this.onAdding = new defaults$3().onAdding;
        /**
         * \@name animationDuration
         */
        this.animationDuration = new defaults$3().animationDuration;
        /**
         * \@name onAdd
    ...
    setUpTextChangeSubscriber() {
        this.inputForm.form
            .valueChanges
            .pipe(debounceTime(this.onTextChangeDebounce), filter((value) => value.item.length > 0))
            .subscribe((value) => {
...
TagInputComponent.propDecorators = {
    "applyFocusOnClick": [{ type: Input },],
    "applyFocusOnAdd": [{ type: Input },],
    "applyFocusOnRemove": [{ type: Input },],
    "applyFocusOnLast": [{ type: Input },],
    ...
    "onTagEdited": [{ type: Output },],
    "_dropdown": [{ type: ContentChild, args: [TagInputDropdown,] },],
    "_virtualizedDropdown": [{ type: ContentChild, args: [TagInputVirtualizedDropdown,] },],
...
export { TagInputModule, TagInputDropdown, TagInputVirtualizedDropdown, DeleteIconComponent, TagInputForm, TagRipple, TagComponent, TagInputComponent, animations as ɵb, TagInputVirtualizedDropdown as ɵc, TagInputAccessor as ɵa, HighlightPipe as ɵe, DragProvider as ɵd };
